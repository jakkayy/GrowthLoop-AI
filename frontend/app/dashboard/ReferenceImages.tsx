...
export default function ReferenceImages({ initialImages }: Props) {
  ...
  async function handleUpload(files: FileList | null) {
    ...
    for (const file of Array.from(files)) {
      ...
      const res = await fetch("/api/user/reference-images", {
        method: "POST",
        body: formData,
      });
      if (res.ok) {
  ...
  async function handleDelete(imageId: string) {
    const res = await fetch(`/api/user/reference-images/${imageId}`, {
      method: "DELETE",
    });
    if (res.ok) {
  ...
  return (
    <div className="rounded-2xl bg-white border border-gray-200 p-6">
      <div className="flex items-start justify-between mb-5">
        <div>
          <h2 className="text-sm font-semibold text-gray-900">Reference Images</h2>
          <p className="text-xs text-gray-500 mt-0.5">รูปต้นแบบที่ AI จะใช้เป็น style reference ในการสร้างรูป</p>
        </div>
        <button
          onClick={() => inputRef.current?.click()}
          disabled={uploading}
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-green-600 text-white text-xs font-medium hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shrink-0"
        >
      ...
      {images.length === 0 ? (
        <div
          ...
          onClick={() => inputRef.current?.click()}
        >
          <p className="text-xs text-gray-400">ยังไม่มีรูป reference — คลิกเพื่ออัปโหลด</p>
        </div>
      ) : (
